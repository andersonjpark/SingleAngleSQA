...
vector<vector<MATRIX<complex<double>,NF,NF> > > UpdateSm(double rminus,double rplus,double Yeminus,double Yeplus,vector<vector<vector<vector<double> > > > Y,vector<vector<vector<MATRIX<complex<double>,NF,NF> > > > C0,vector<vector<vector<vector<double> > > > A0,vector<vector<MATRIX<complex<double>,NF,NF> > > Smprior);

vector<vector<vector<MATRIX<complex<double>,NF,NF> > > > UpdateC(double r,double Ye);

vector<vector<vector<vector<double> > > > UpdateA(vector<vector<vector<MATRIX<complex<double>,NF,NF> > > > C,vector<vector<vector<MATRIX<complex<double>,NF,NF> > > > C0,vector<vector<vector<vector<double> > > > A0);
...
vector<vector<MATRIX<complex<double>,NF,NF> > > UpdateSm(double rminus,double rplus,double Yeminus,double Yeplus,vector<vector<vector<vector<double> > > > Y,vector<vector<vector<MATRIX<complex<double>,NF,NF> > > > C0,vector<vector<vector<vector<double> > > > A0,vector<vector<MATRIX<complex<double>,NF,NF> > > Smprior)
	    ...
	    for(int i=0;i<=NE-1;i++)
               { Hf=HfV[matter][i]+VfMSW[matter];
            	 ...
            	 Sm[matter][i]=UU *W(Y[matter][i][msw])*B(Y[matter][i][msw]) *W(Y[matter][i][si])*B(Y[matter][i][si]) * Smprior[matter][i];

            	 Hfbar=HfV[antimatter][i]+VfMSW[antimatter];
            	 kkbar=kbar(Hfbar);
            	 dkkbar=deltakbar(Hfbar);
                 UUbar=Conjugate(U(dkkbar,C0[antimatter][i],A0[antimatter][i]));
            	 Sm[antimatter][i]=UUbar *W(Y[antimatter][i][msw])*B(Y[antimatter][i][msw]) *W(Y[antimatter][i][si])*B(Y[antimatter][i][si]) * Smprior[antimatter][i];
		}
...
vector<vector<vector<MATRIX<complex<double>,NF,NF> > > > UpdateC(double r,double Ye)
          { vector<MATRIX<complex<double>,NF,NF> > VfMSW(NM);
            ...
            vector<vector<vector<MATRIX<complex<double>,NF,NF> > > > CC(NM,vector<vector<MATRIX<complex<double>,NF,NF> > >(NE,vector<MATRIX<complex<double>,NF,NF> >(NF)));
...
vector<vector<vector<vector<double> > > > UpdateA(vector<vector<vector<MATRIX<complex<double>,NF,NF> > > > C,vector<vector<vector<MATRIX<complex<double>,NF,NF> > > > C0,vector<vector<vector<vector<double> > > > A0)
      { vector<vector<vector<vector<double> > > > A(NM,vector<vector<vector<double> > >(NE,vector<vector<double> >(NF,vector<double>(NF))));
