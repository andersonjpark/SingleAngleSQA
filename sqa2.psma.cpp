...
void Outputvsr(ofstream &fout, ofstream &foutP,ofstream *foutPvsr,ofstream *foutFvsr,double r,vector<vector<vector<vector<double> > > > Y,vector<vector<vector<MATRIX<complex<double>,NF,NF> > > > C0,vector<vector<vector<vector<double> > > > A0,vector<vector<MATRIX<complex<double>,NF,NF> > > Scumulative);
...
foutS.open((outputfilename+"/"+"S"+patch::to_string(rmin)+"-"+patch::to_string(rmax)+"km"+patch::to_string(accuracy)+"ACC"+patch::to_string(NE)+"NE"+patch::to_string(nt)+note+".txt").c_str());
           foutS.precision(12);
foutS.flush();
 foutP.open((outputfilename+"/"+string("2p")+patch::to_string(rmin)+"-"+patch::to_string(rmax)+"km"+patch::to_string(accuracy)+"ACC"+patch::to_string(NE)+"NE"+patch::to_string(nt)+note+".txt").c_str());
           foutP.precision(12);
foutP.flush();
fout.open((outputfilename+"/"+patch::to_string(rmin)+"-"+patch::to_string(rmax)+"km"+patch::to_string(accuracy)+"ACC"+patch::to_string(NE)+"NE"+patch::to_string(nt)+note+".txt").c_str());
fout.precision(12);
fout.flush();
...
foutP<<real(VfSI[matter][e][e])<<"\t"<<imag(VfSI[matter][e][e])<<"\t"<<real(VfSI[matter][mu][mu])<<"\t"<<imag(VfSI[matter][mu][mu])<<"\t";//3,4,5,6
fout<<real(VfSI[matter][e][e])<<"\t"<<imag(VfSI[matter][e][e])<<"\t"<<real(VfSI[matter][mu][mu])<<"\t"<<imag(VfSI[matter][mu][mu]);
fout<<real(VfSI[antimatter][e][e])<<"\t"<<imag(VfSI[antimatter][e][e])<<"\t"<<real(VfSI[antimatter][mu][mu])<<"\t"<<imag(VfSI[matter][mu][mu]);
